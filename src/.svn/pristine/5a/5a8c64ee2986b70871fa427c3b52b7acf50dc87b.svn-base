...
public class AttachmentController {
	...
	public void downloadAttachmentAsPdf(String id, HttpServletResponse response, HttpServletRequest request) {
	    ...
	    if(attachment != null) {
            //判断文件是否为图片格式
            String filenamepd = attachment.getName();
            String regex = "[(.JPEG)|(.jpeg)|(.JPG)|(.jpg)|(.GIF)|(.gif)|(.BMP)|(.bmp)|(.PNG)|(.png)]$";
            Pattern p = Pattern.compile(regex);
            Matcher m = p.matcher(filenamepd);
...
//	@RequestMapping(value = "/qmfj", method = RequestMethod.POST)
//	@ResponseBody
//	private String getDyImageStr() {
//		String qmfj = attachmentService.getDyImageStr();
//		return qmfj;
//	}
	 ...
	 * @param response
	 */
	@RequestMapping(value = "/qmxs", method = RequestMethod.GET)
	public void showAvatar(HttpServletResponse response) {
		List<AttachmentVo> attachmentVos = userService.getQmAttachment();
		int size = attachmentVos.size();
		if(size!=0){
		String imageFile = attachmentVos.get(size - 1).getPath() + "/" + attachmentVos.get(size - 1).getId();
...
//				byte[] avatarByte = IOUtil.inputDataFromFile(avatar.getPath() + File.separator + avatar.getId());
		byte[] avatarByte = IOUtil.inputDataFromFile(imageFile);
		System.out.println(avatarByte);
		try {
...
//		}
	 ...
	 *
	 */
	@RequestMapping(value = "/qmxsById", method = RequestMethod.GET)
	public void showAvatarById(String Id, HttpServletResponse response) {
		List<AttachmentVo> attachmentVos = userService.getQmAttachmentByUserId(Id);
		int size = attachmentVos.size();
		if(size!=0){
			String imageFile = attachmentVos.get(size - 1).getPath() + "/" + attachmentVos.get(size - 1).getId();
//		UserVo user = userService.getUserById(userId);
