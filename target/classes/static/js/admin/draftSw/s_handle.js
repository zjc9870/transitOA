...
for(var i=0; i<tabs.length; i++) {
    tabs[i].onclick = function () {
        ...
        AjaxTool.post('draftSw/tabRequest', {
                tab: this.id, ym: 'swbl'},function (data) {
                if(data.success) {
                    var str = "";
                    ...
                    for(var i=0;i<drafts.length;i++) {
                        ...
                        str += "<td><div onclick='seeBl(\""+ drafts[i].id +'\",\"' + tabId +"\")'>查看</div></td>";
                        str += "</tr>";
                    }
                    var mTable = $('#s-handle-table').DataTable();
                    mTable.destroy();
...
function init() {//dataTable初始化
    mTable=DatatableTool.initDatatable("s-handle-table", [ {
        'orderable' : false,
        'targets' : [ 4 ]
    }, {
        "searchable" : false,
